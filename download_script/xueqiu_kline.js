...
function company_kline(id,code,lv,callback){
    ...
    function xueqiuURI(timestamp){
        let uri = `https://stock.xueqiu.com/v5/stock/chart/kline.json?symbol=${code}&begin=${timestamp}&period=${lv}m&type=before&count=-${ucount[lv]}&indicator=kline,macd`;
        return uri;
    }
     ...
     * 第一步找到数据库中最近的一条记录
     */
    connection.query(`select max(timestamp) as r from k${lv}_xueqiu where id=${id};`,(error, results, field)=>{
        /**
         * 第二步覆盖最开头的数据
         */
                        ...
                        if(sl.data && sl.data.column && sl.data.item && sl.data.column.length===columns.length && sl.data.item.length && isCorrect(sl.data.column) && nextdate!=sl.data.item[0][0]){
                            ...
                            for(let i=items.length-1;i>=0;i--){ //从最近的向过去处理
                                ...
                                if(head_date && date.getTime()===head_date.getTime()){
                                    ...
                                    sqlStr = `update k${lv}_xueqiu set ${str_pairs(it)} where id=${id} and timestamp='${dateString}'`;
 ...
 */
function download_kline(lv,done){
    paralle_companys_task('id,code',10,com=>cb=>{
        company_kline(com.id,com.code,lv,cb);
    }).then(usetime=>{
        if(done)done();
    }).catch(err=>{
        console.log(err);
