...
function k15_company(id,code,callback){
    ...
    function xueqiuURI(timestamp){
        ...
        let uri = `https://stock.xueqiu.com/v5/stock/chart/kline.json?symbol=${code}&begin=${timestamp}&period=15m&type=before&count=-32&indicator=kline`;
        return uri;
    }
                        ...
                        if(sl.data && sl.data.column && sl.data.item && sl.data.column.length===columns.length && sl.data.item.length && isCorrect(sl.data.column) && nextdate!=sl.data.item[0][0]){
                            ...
                            for(let i=items.length-1;i>=0;i--){ //从最近的向过去处理
                                ...
                                if(head_date && date.getTime()===head_date.getTime()){
                                    ...
                                    //sqlStr = `update k15_xueqiu set ${cols.slice(1,-1).join()} where id=${id} and timestamp='${dateString}'`;
                                    sqlStr = `update k15_xueqiu set ${str_pairs(it)} where id=${id} and timestamp='${dateString}'`;
                                    ...
                                    //let cols = it.map((v)=>`${v}`);
                                    //datas.push(`(${id},'${dateString}',${cols.slice(1,-1).join()})`);
